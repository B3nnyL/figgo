	...
	try {
		...
		if (response.status === 200) {
			...
			switch (response.status) {
				case 403:
					throw new Error(
						`Ops... ${response.status} - Figgo cannot authenticate you`
					);
				case 404:
					throw new Error(
						`Ops... ${response.status} - Figgo cannot find board (id:${board})`
					);
				default:
					break;
			}
	...
	if (data !== undefined) {
		...
		for (const i in colorBlocks) {
			if (colorBlocks[i].fills[0].type === "SOLID") {
				...
				switch (type) {
					case "scss":
						array.push(newColor.SCSSColor);
						break;
					case "js":
						array.push(newColor.JSONColor);
						break;
					case "css":
						array.push(newColor.CSSVariableColor);
						break;
					default:
						break;
				}
	...
	if (data !== undefined) {
		...
		for (const i in spaceBlocks) {
			if (spaceBlocks) {
				...
				switch (type) {
					case "scss":
						array.push(newSpace.SCSSValue);
						break;
					case "js":
						array.push(newSpace.JSONValue);
						break;
					case "css":
						array.push(newSpace.CSSVariableValue);
						break;
					default:
						break;
				}
		...
		for (const i in typoBlocks) {
			if (typoBlocks[i]) {
				...
				switch (type) {
					case "scss":
						array.push(
							newTypo.SCSSFontFamily,
							newTypo.SCSSFontSize,
							newTypo.SCSSFontWeight,
							newTypo.SCSSLetterSpacing,
							newTypo.SCSSLineHeight
						);
						break;
					case "js":
						array.push(
					...
					case "css":
						array.push(
							newTypo.CSSVariableFontFamily,
							newTypo.CSSVariableFontSize,
							newTypo.CSSVariableFontWeight,
							newTypo.CSSVariableLetterSpacing,
							newTypo.CSSVariableLineHeight
						);
						break;
					default:
						break;
